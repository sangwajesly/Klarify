...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!identifier.trim()) {
      setError("Please enter your phone number or email."); return;
    }
    if (usingPhone) {
      const digits = identifier.replace(/\D/g, "");
      if (digits.length < 9) {
        setError("Phone number must be at least 9 digits."); return;
      }
    ...
    try {
      ...
      // If phone user doesn't have an account yet, guide them to sign up
      if (usingPhone && (msg.includes("Invalid login") || msg.includes("invalid_credentials"))) {
        setError("No account found for this number. Please sign up first, or check your password.");
      } else {
