...
const ProgramDetails = () => {
      ...
      try {
        ...
        const related = all
          .filter(
            (p) =>
              p.id !== id &&
              (p.university === data.university || p.faculty === data.faculty),
          )
          .slice(0, 3);
  ...
  const programSchema = {
    ...
    description:
      ...
      `${program.name} offered by ${program.university}`,
    provider: {
      ...
      <main className="pt-2 pb-20">
        ...
        <div className="bg-slate-900 text-white rounded-2xl p-6 sm:p-9 shadow-lg border border-slate-800 relative overflow-hidden mb-8">
          <div className="flex flex-col md:flex-row justify-between items-start gap-6 relative z-10">
            <div className="space-y-3 flex-1">
              {/* University pill */}
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 dark:bg-slate-900/10 border border-white/15 text-orange-300 text-xs font-semibold">
                <Building2 size={14} />
                {program.university}
              </div>
              ...
              <div className="flex flex-wrap items-center gap-4 text-xs sm:text-sm text-slate-300 pt-1">
                ...
                {program.degreeObtained && (
                  <div className="flex items-center gap-1.5">
                    ...
                    <span>{program.degreeObtained}</span>
                  </div>
                )}
                <div className="flex items-center gap-1.5">
              ...
              <button
                ...
                className={`p-3 rounded-xl border transition-all flex items-center gap-2 text-xs font-bold ${
                  isSaved
                    ...
                    : "bg-white/10 dark:bg-slate-900/10 border-white/20 text-white hover:bg-white/20 dark:bg-slate-900/20"
                }`}
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-12">
          ...
          <div className="lg:col-span-8 space-y-8">
            {/* Overview */}
            <section className="bg-white dark:bg-slate-900 rounded-2xl p-6 sm:p-7 border border-slate-100 dark:border-slate-800 shadow-xs">
              ...
              <p className="text-slate-600 dark:text-slate-400 text-sm sm:text-base leading-relaxed">
                {program.descriptions ||
                  `The ${program.name} degree offered by ${program.university} (${program.faculty || "Faculty"}) is designed to equip students with theoretical knowledge and practical skills for professional excellence.`}
              </p>
              ...
              {program.tags && program.tags.length > 0 && (
                <div className="mt-6 pt-5 border-t border-slate-100 dark:border-slate-800 flex flex-wrap gap-2">
                  {program.tags.map((tag, idx) => (
                    <span
                      key={idx}
                      className="px-3 py-1 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 text-xs font-semibold rounded-lg"
                    >
                      #{tag}
                    </span>
                  ))}
            ...
            {program.requiresConcours && program.examDetails && (
              <section className="relative overflow-hidden rounded-2xl border border-blue-200 bg-linear-to-r from-blue-50 via-cyan-50/60 to-white p-6 sm:p-7 shadow-sm">
                ...
                <div className="grid grid-cols-3 gap-3 mb-6">
                  <div className="bg-white/80 dark:bg-slate-900/80 rounded-xl p-3 border border-blue-100 text-center">
                    <span className="block text-[10px] uppercase font-bold text-blue-600/80 mb-0.5">
                      Exam Month
                    </span>
                    <span className="block text-sm font-bold text-slate-900 dark:text-white">
                      {program.examDetails.month || "June / July"}
                    </span>
                  </div>
                  <div className="bg-white/80 dark:bg-slate-900/80 rounded-xl p-3 border border-blue-100 text-center">
                    <span className="block text-[10px] uppercase font-bold text-blue-600/80 mb-0.5">
                    ...
                    <span className="block text-sm font-bold text-slate-900 dark:text-white">
                      {program.examDetails.deadline || "Varies"}
                    </span>
                  </div>
                  <div className="bg-white/80 dark:bg-slate-900/80 rounded-xl p-3 border border-blue-100 text-center">
                    <span className="block text-[10px] uppercase font-bold text-blue-600/80 mb-0.5">
                    ...
                    <span className="block text-sm font-bold text-slate-900 dark:text-white">
                      {program.examDetails.fee || "20,000 XAF"}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <button
                    ...
                    className="inline-flex items-center justify-center gap-2 bg-white dark:bg-slate-900 hover:bg-blue-50 text-blue-700 border border-blue-200 text-xs font-bold px-4 py-2.5 rounded-xl transition-colors shadow-xs"
                  >
              ...
              {program.careers && program.careers.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {program.careers.map((career, idx) => (
                    <div
                      key={idx}
                      className="flex items-center gap-2.5 p-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 text-xs font-semibold text-slate-700 dark:text-slate-300"
                    >
                      <CheckCircle2
                        size={16}
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 italic">
                  No specific career roles mapped yet.
                </p>
              )}
            </section>
          </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            {/* Prerequisites Card */}
            <div className="bg-slate-50 dark:bg-slate-950 rounded-2xl p-6 border border-slate-200/80 dark:border-slate-700/80">
              ...
              <div className="space-y-4 text-xs sm:text-sm text-slate-600 dark:text-slate-400">
                <div>
                  <span className="block font-semibold text-slate-800 dark:text-slate-200 mb-1">
                    Required A-Level Subjects:
                  </span>
                  <div className="p-3 bg-white dark:bg-slate-900 rounded-xl border border-slate-200/60 dark:border-slate-700/60 font-medium text-slate-700 dark:text-slate-300">
                    {program.requiredALSubjects ||
                      "GCE A-Level Passes in relevant Arts or Science subjects."}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <p className="text-slate-700 dark:text-slate-300 font-bold">{program.duration}</p>
                </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-100 dark:border-slate-800 shadow-xs text-center">
              <Building2 className="mx-auto text-orange-500 mb-2" size={32} />
              <h4 className="font-bold text-slate-900 dark:text-white text-sm mb-1">
                {program.university}
              </h4>
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">{program.faculty}</p>
              <Link
                to={`/universities`}
        ...
        {relatedPrograms.length > 0 && (
          <section className="pt-8 border-t border-slate-200 dark:border-slate-700">
            <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-6">
              Related Programs You Might Like
            </h2>
