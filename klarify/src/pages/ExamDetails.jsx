...
import { ArrowLeft, AlertCircle, CalendarDays, FileText, BadgeDollarSign, GraduationCap } from 'lucide-react';
    ...
    <div className="bg-white/60 border border-slate-200/70 rounded-xl px-4 py-3">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <div className="text-sm font-bold text-slate-900 break-words">{Array.isArray(value) ? value.join(', ') : value}</div>
        </div>
      </div>
    </div>
  );
      ...
      <Layout>
        <div className="max-w-3xl mx-auto px-4 py-12">
          <div className="flex items-center gap-2 text-sm text-slate-600">
            <ArrowLeft size={16} />
            <button className="underline" onClick={() => navigate('/results')}>Back to results</button>
          </div>
          <div className="mt-6 bg-white/60 border border-slate-200 rounded-2xl p-6">
            <AlertCircle className="text-orange-600" size={22} />
            <p className="mt-3 text-slate-700 font-medium">No exam details were provided.</p>
          </div>
        </div>
      </Layout>
    );
    ...
    <Layout>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        <div className="flex items-center gap-2 text-sm font-medium text-slate-600 mb-6">
          <ArrowLeft size={16} />
          <button className="hover:text-orange-700 transition-colors" onClick={() => navigate('/results')}>Back to results</button>
        </div>
        ...
        <div className="bg-slate-900 text-white rounded-3xl p-6 sm:p-8 relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <h1 className="mt-4 text-2xl sm:text-3xl font-extrabold">{examDetails.name || 'Exam Details'}</h1>
            ...
            {(examDetails.month || examDetails.deadline || examDetails.fee) && (
              <div className="mt-4 flex flex-col sm:flex-row gap-3">
                <div className="bg-white/10 border border-white/15 rounded-2xl px-4 py-3">
                  <div className="text-[11px] uppercase tracking-wider text-slate-300 font-semibold">Exam Month</div>
                  <div className="text-sm font-bold text-white">{examDetails.month || '-'}</div>
                </div>
                <div className="bg-white/10 border border-white/15 rounded-2xl px-4 py-3">
                  <div className="text-[11px] uppercase tracking-wider text-slate-300 font-semibold">Deadline</div>
                  <div className="text-sm font-bold text-white">{examDetails.deadline || '-'}</div>
                </div>
                <div className="bg-white/10 border border-white/15 rounded-2xl px-4 py-3">
                  <div className="text-[11px] uppercase tracking-wider text-slate-300 font-semibold">Fee</div>
                  <div className="text-sm font-bold text-white">{examDetails.fee || '-'}</div>
                </div>
              </div>
            )}

            {examDetails.summary && (
              <p className="mt-5 text-slate-200 leading-relaxed">{examDetails.summary}</p>
            )}
          </div>
        </div>
        ...
        {(examDetails.registration_procedure && Array.isArray(examDetails.registration_procedure) && examDetails.registration_procedure.length > 0) && (
          <div className="mt-6 bg-white/70 backdrop-blur rounded-3xl border border-slate-200/70 p-6">
            <h2 className="text-lg font-extrabold text-slate-900">Registration Procedure</h2>
